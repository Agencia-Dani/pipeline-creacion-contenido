...
async function exigirAdminDeEquipo() {
  const sesion = await exigirTenant("ajustes");
  if (!puedeAdministrarEquipo(sesion.rol)) return null;
  return sesion;
}

export async function invitar(form: {
  email: string;
  nombre: string;
  rol: string;
}): Promise<Resultado> {
  const sesion = await exigirAdminDeEquipo();
...
export async function cambiarRol(usuarioId: string, rol: string): Promise<Resultado> {
  const sesion = await exigirAdminDeEquipo();
  if (!sesion) return { ok: false, mensaje: "No podés administrar el equipo de esta empresa." };
  const { usuario, ctx, cockpit } = sesion;
...
export async function quitarAcceso(usuarioId: string): Promise<Resultado> {
  const sesion = await exigirAdminDeEquipo();
  if (!sesion) return { ok: false, mensaje: "No podés administrar el equipo de esta empresa." };
  const { usuario, ctx, cockpit } = sesion;
