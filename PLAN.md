...
| `Workflows/workflow-descubrimiento-referentes/` | **Máquina**: descubrimiento de referentes n8n (24 nodos; ADR-020). Cron semanal → semillas = referentes que mejor convierten (`v_senal_seleccion`) → sugeridos del propio IG (Apify `relatedProfiles`) → dedup → vetting Haiku contra criterios → propuestas a la tabla `Referentes propuestos` + promoción de aprobados a `Referentes`. No toca el motor de reels. | ✅ Construido · ❌ **sin importar en n8n** · placeholders `<<...>>` por cliente |
