...
- **Las 2 lecturas de Supabase** (señal selección → procesados) van enhebradas **después
  ...
  `external_id` de la corrida (`in.(…)`, dedup acotado #5) — ya no `limit=20000`. **Es fail-closed**
  (ADR-029): si el GET falla, el run aborta en vez de re-entregar todo lo ya visto.
...
| 19 | Leer procesados | http GET | Supabase `processed_items` filtrado a los `external_id` de **esta corrida** (`external_id=in.(…)`, dedup acotado #5) → el set de dedup. Ya no `limit=20000`. **Fail-closed (ADR-029):** GET caído aborta el run (lo barre el zombie sweep), no re-entrega. |
| 19b | Leer feed vivo | http GET | **(nuevo, ADR-029)** GET paginado a Airtable `Candidatos` (solo `external_id`) → la última línea de dedup contra el feed vivo. En serie tras 19. **Fail-open** (defensa secundaria). |
| 20 | Heat-score v1 | code | **Prescore métrico** = `peso_views·pct(views) + peso_likes·pct(likes) + peso_eng·pct(eng)` por proyecto; × `(1+boost_idioma)` si `guessLang(caption+bio)≠es`; × `(1+sel)` con `sel` = señal por referente (`v_senal_seleccion`; mono-eje desde ADR-019). **Piso duro** `min_views`/`min_likes` y **dedup**: `seen` = `processed_items` ∪ **feed vivo** (ADR-029; tripwire que aborta si `processed_items` trae `.error`); `cap_top_n` recorta por videos distintos. Emite `heat_score`, `idioma_guess`, `viral_por_tamano`. Salidas en orden `[Preparar procesados, Transcribir]` (memoria primero). Lee 17, 18, 19, 19b + Config⊕ajustes. |
