...
propio id) y solo barre `en_curso` más viejos que `ventana_corrida_min` (45); el guard bloquea la
...
- **El guard single-flight (C.3, ADR-023) aplica a los 3 triggers**, no solo al webhook (decisión de
  ...
  Vivo vs. zombie lo decide `ventana_corrida_min` (Config, 45): `en_curso` más joven = corrida viva
...
- **`Heat-score v1` tiene dos salidas:** una al pipeline de enriquecimiento (Transcribir→…→candidato)
  ...
  aunque el gate lo haya descartado. **Orden de ramas (ADR-029): `Preparar procesados` va primera** —
  con execution order v1 (depth-first) la memoria se graba **antes** de entregar (antes gastaba Supadata
  y podía fallar en silencio, como el 20/07). El diagrama de arriba muestra Transcribir a la izquierda
  por legibilidad del pipeline, pero la rama que corre primero es la de la memoria.
...
| 25 | Preparar batch Airtable | code | Arma `records[]` de Airtable (`fields` + links `proyecto`/`voz` + `thumbnail` attachment + **`external_id`**, ADR-029), en **batches de 10**, `typecast:true`, `estado:'nuevo'`. |
...
| 2 | Ejecutar manual | manualTrigger | Execute a mano. |
| 3 | Config | set | `airtable_base_id`, `supabase_url`, `instance_id`, `sheet_id`, `sheet_tab` (placeholders `<<…>>`) + `min_muestra_referente` 10, `min_muestra_destilar` 4 (ADR-022) + `ventana_corrida_min` 45 (matiz D.2: frontera vivo/zombie al contar `runs_ok/fallo` — mismo nombre y semántica que en el motor). |
