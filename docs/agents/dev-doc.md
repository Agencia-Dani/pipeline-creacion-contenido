...
- **`Transcribir (Supadata)` es un Code node SERIAL bajo el watchdog del task runner**
  ...
  - **Mitigación de tiempo (no de costo):** desde el cierre 31 el nodo tiene **presupuesto propio**
    (`presupuesto_transcribir_s` 3000, Config dev-only): si el loop lo excede, corta y el resto pasa
...
| 21 | Transcribir (Supadata) | code | Transcribe cada **video distinto** del top_n UNA vez (dedup por `external_id`, reparte a las copias del fan-out; Supadata `&text=true&mode=auto`), throttle 1s, trunca 6000 chars. `idioma_detectado` = `lang` de Supadata (primario) → fallback `guessLang(transcript)` → `idioma_guess`. **Presupuesto de tiempo** (`presupuesto_transcribir_s` 3000, cierre 31; subido de 780 en el cierre 54 tras medir ~27s/video — 780 cubría solo ~28 de 84): si el loop lo excede, corta y el resto pasa sin transcript — evita que el watchdog del task runner (3600s desde el cierre 54) mate el nodo y el run. **Fail-open** (sin transcript, sigue). `<SUPADATA_API_KEY>`. |
