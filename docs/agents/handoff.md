...
> |---|---|---|---|
> | **F1** | ✅ **RESUELTO en código (cierre 12).** Las 2 ramas iban al mismo input → n8n corría el Normalizador 2× (pasadas separadas). **Fix:** `Merge IG`/`Merge TT` (append) ANTES de cada Normalizador. Validador 1089/0. Falta re-import + verificar 1× en n8n. | igreels=40 bayavoce · ighashtags=30 · normalizar-ig=30 (≠70 → no concatenó) · normalizar-tt=1 | 🟢 fix aplicado |
> | **F2** | Eje IG-Hashtag rankea ciego: items sin `followersCount`/`videoViewCount` (solo `metaData`) → `seguidores/bio/views=0` (30/30); fix #22 no los cubre. Supadata no transcribió ninguno (`script_len=0`), idioma hi/pl/en. | normalizar-ig seg=0 30/30 · transcribir 3× script_len=0 | 🟠 |
> | **F3** | IG-Reels sí funciona end-to-end (9 candidatos, rel_score 0.75–0.95, on-topic) PERO los 40 reels son TODOS @bayavoce → 1 de 7 referentes aporta (#24). | igreels owners={bayavoce:40} · armar-candidato 9× bayavoce | 🟠 |
> | **F4** | TikTok aportó 0 pese a 40 items bien formados y on-topic (fans 1.8M, "#relationship"). El más caro. **Colateral de F1** → esperado resuelto por el fix de los Merge; verificar en el re-run. | apify-tiktok=40 ok · ningún TikTok downstream | 🟢 esperado por F1 |
> | **F5** | `Gate` emitió `[{}]` sobre los 3 de hashtag (transcript vacío → fallback caption). Confirma la nota de Mani: gate sin output, workflow no frenó. | gate-de-relevancia.json=`[{}]` | 🟡 verificar |
...
>
> **Bloque B — Normalización + asignación (el lead caliente).**
> - **B1 `normalizar-ig` (30): 🟡 PARCIAL.** El snapshot es **solo la pasada Hashtag** (F1): `seguidores=0` 30/30 (los items hashtag no traen follower data — F2, **no** bug de mapeo), 0 fotos (Video, #23). **Falta** ver el mapeo de la pasada **IG-Reels** (#22 seguidores poblados) en el run post-F1.
> - **B2 `normalizar-tt` (1 vacío) = F1: ✅ RESUELTO (cierre 12).** Cableado real verificado en `workflow.json`: IG Reels+IG Hashtag→`Normalizar IG` in#0; TikTok+Perfil→`Normalizar TT` in#0 → n8n corre **1×/conexión** → 2 pasadas separadas. **Fix:** `Merge IG`/`Merge TT` (append) ANTES de cada Normalizador (34→36 nodos, validador 1089/0). Falta re-import + verificar 1× en el log de n8n.
> - **B3 `asignar-proyecto+voz` (30): ⬜ PENDIENTE.** Match por referente/keyword, guarda de recencia, cuántos por proyecto, todos IG. (`outputs/asignar-proyecto+voz.json` ya en disco.)
>
> **Bloque C — Relevancia + scoring. ⬜ PENDIENTE (re-leer del run post-F1).**
> - **C1 `pretrim-relevancia` (30→3) [lead #3]: ⬜** razones de descarte (el código **no logea** descartes — instrumentar o leer prompt/criterios), si Haiku respondió bien, fail-open.
> - **C2 `heatscore-v1` (3): ⬜** ranking, `idioma_guess`, `viral_por_tamano`, `top_n` por proyecto.
> - **C3 `transcribir-supadata` (3) + `traducir-claudehaiku` (3): ⬜** idioma detectado, literalidad, fail-open. (F2: Supadata no transcribió los 3 de hashtag, `script_len=0`; `traducir` sacó 0/73/442 chars sobre transcript=0.)
> - **C4 `gate-de-relevancia` (vacío) + `armar-candidato` (9) [bug #2] = F5: 🟡 PARCIAL.** Explicado por F1: los **9 son la pasada Reels** (`rel_score` 0.75–0.95 presente → **sí** pasaron el gate ahí); el `gate=[{}]` es la pasada **Hashtag** (transcript vacío→fallback caption→dropeados). **Falta** confirmar en el run post-F1 que armar-candidato no reciba items sin `relevancia_score`.
>
> **Bloque D — Cierre + cross-check en vivo. ⬜ PENDIENTE (PAT Airtable + service_role Supabase en mano, A ROTAR).**
> - **D1 Airtable `Candidatos`:** ¿9 records de este run? `relevancia_score`/`relevancia_razon` no-null, links proyecto/voz, thumbnail.
> - **D2 Supabase `runs`:** `metricas {colectados, filtrados, outputs}` + estado (¿`ok` o zombie `en_curso`?).
> - **D3 Supabase `outputs` (draft) + `processed_items`:** qué se registró, coherencia con el embudo.
> - **D4 Veredicto:** lista bugs/mejoras/conexiones erradas con severidad, **separando** *bug de código* (F1✅) de *config Airtable* (#24 `ig_results_limit`, sembrar referentes TikTok) de *límite del actor* (F2 métricas de hashtag).
>
