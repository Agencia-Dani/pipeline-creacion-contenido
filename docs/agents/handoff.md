...
**2026-07-15 (cierre 39) — Prep de reunión con el equipo de redes + auditoría del scoring del buscador (Mani + Claude).** Pedido de Mani: quedar listo para una reu con redes (entender backend nodo a nodo, qué ve el equipo en Airtable, y todo lo del medio). **NO es trabajo de producto — es documentación/orientación.** **(1) Guía de reunión NUEVA:** [docs/agents/guia-reunion-redes.md](guia-reunion-redes.md) — un solo mapa que conecta las 3 capas (3 workflows condensados nodo-a-nodo ↔ 9 tablas mapeadas a las páginas del Cockpit Redes ↔ qué hace/no hace el equipo) + Q&A anticipado + el **caveat clave para no prometer de más** (M2/costos/contadores Apify están en el repo pero **no re-importados en n8n** → varias columnas vacías hasta re-import + 1 ciclo) + los **3 gaps de UI** pendientes (veredicto solo-lectura en Descartes, páginas de Métricas sin curar, precision como decimal). dev-doc y onboarding NO se tocaron (ya están al día; reescribir era churn). **(2) Auditoría del scoring del descubrimiento (leyendo el código, no la doc):** pregunta de Mani = "¿los referentes nuevos se evalúan por similitud y métricas? ¿el score es correcto para el workflow?". **Hallazgo verificado en los 4 code nodes** (`Agregar sugeridos`/`Vetting relevancia`/`Vetting TikTok`/`Armar propuestas`): `afinidad` (0–1) = **SOLO el juicio semántico de Haiku** (bio+captions IG / bio-sola TT vs criterios Proyecto⊕Voz, precision-biased). **Similitud** solo *genera* candidatos (IG `relatedProfiles` + frecuencia; TT lookalike `score`) y **desempata** (freq), no puntúa. **Métricas (seguidores/views/likes)** se recolectan y se muestran pero el score **las ignora a propósito** (ambos prompts: "importa lo que publica, no su fama") — el prescore métrico es trabajo del motor per-video. **Veredicto:** para IG el diseño es correcto (el referente es fuente permanente; la viralidad la re-juzga el motor; el score solo filtra ≥0.6 + ordena top-10 y **no viaja al motor**). **3 debilidades reales flageadas:** (a) TikTok se juzga casi a ciegas (sin captions, solo bio+métricas → `afin` TT menos confiable); (b) IG+TT comparten un solo ranking + cap de 10 → un 0.7 TT (bio) puede superar a un 0.7 IG (captions), apples-to-oranges; (c) sin piso de seguidores (una cuenta chica muy del-tema entra con peso pleno). **Mejora anotada (ya en ADR-020 §8):** 2ª pasada `clockworks` para captions TT; piso de seguidores = knob de 1 línea si importa el alcance. **Documentado** en una sección nueva "Cómo se puntúa una propuesta" del [README del buscador](../../Workflows/workflow-descubrimiento-referentes/README.md). **Archivos de esta sesión:** `guia-reunion-redes.md` (nuevo, sin trackear), README del descubrimiento (+sección scoring). **Sin cambios de código ni de base viva.** El working tree sigue arrastrando los 12 archivos staged de cierre 38 (sin commitear, Mani decide). **PENDIENTE: todo lo de cierre 38 sigue igual** (re-import de los 3 workflows, los 5 fixes de UI de Airtable, rotar credenciales, sembrar TT, aprobar propuestas).

**2026-07-15 (cierre 38) — Auditoría completa + reconciliación repo↔live + fix de docs, antes de la sesión de Airtable (Mani + Claude).** Pedido de Mani: "envolver la cabeza" alrededor de todo lo que pasó y dejar los docs exactos a lo hecho y live. **NO es trabajo nuevo de producto — es la pasada de auditoría** que ordena lo que dejaron los cierres 36–37. **Reconciliación (verificado por MCP + estático):** el working tree (12 archivos) = **ADR-021 bis en código** (los 3 `workflow.json`: motor cuenta `apify_ig/tt`, archivado **36→37 nodos** con `Leer runs descubrimiento` + fila `DESCUBRIMIENTO` + Apify, descubrimiento cuenta sus 3 actores) **+ enmienda ADR-010** (Voz = contexto) **+ sobras de M2**. Validador **1194/0**, grafos limpios (motor 33 / archivado 37 / descubrimiento 27, 0 refs rotas, 0 huérfanos). **Airtable live (`appkdNLlN1v6XdKHn`):** existen TODOS los campos M2 y TODAS las columnas ADR-021 bis (incl. 9 fórmulas `costo_*`) con helper text; `diagnostico` backfilleado (cierre 37); **página *Costos* existe como BORRADOR sin publicar** (`pagjPe0IcSIx5TGXh`); renombres de página aplicados. **Pendientes ya CERRADOS (tachar):** 5 opciones basura de `idioma` (queda es/en/pt/it/fr/otro), renombres de páginas, `diagnostico` en layout, columnas de costo creadas. **🔴 Gaps NUEVOS que el cierre 37 no flageó (críticos para que el equipo use el cockpit):** (1) **`veredicto` en la página *Descartes* está solo-lectura** (`isEditable:false`) → el equipo NO puede marcar "era bueno", el loop de auditoría de descartes está inutilizable (arrastre del cierre 30); (2) **las 2 páginas de Métricas NO están curadas** — *Salud del Sistema* y *Métricas de Calidad* muestran casi los mismos campos; falta el embudo/runs en Salud y la separación por audiencia de ADR-021 bis; (3) `precision` se muestra como decimal, no %. **Decisiones de Mani (2026-07-15):** re-import **sigue pendiente**; **costo = página *Costos* dedicada** (no sección de Salud). **Fix de docs HECHO (staged, sin commitear):** contrato §8 (nombres de página live + *Costos* como borrador a publicar), ADR-021 bis pto 3 (tres páginas, *Costos* dedicada). **➡️ PRÓXIMA SESIÓN = AIRTABLE, dejar listo para publicar (todo UI):** (1) hacer `veredicto` editable en *Descartes*; (2) curar *Métricas de Calidad* (counts + precision% + diagnostico, ocultar decimales) y *Salud del Sistema* (headline + embudo + runs); (3) **publicar la página *Costos*** + sumarle desglose Apify/DESCUBRIMIENTO + fijar decimales de las fórmulas (la API no lo hace); (4) precision como %; (5) filtro *Referentes Buscados* → `estado=propuesto`; (6) revisión final de helper text tabla por tabla. **PENDIENTE (Mani, manual, arrastres):** **RE-IMPORT de los 3 workflows en n8n** (mapear credenciales a nodos nuevos + `<ANTHROPIC_API_KEY>`/`<SUPADATA_API_KEY>`) → recién ahí M2 + ADR-021 bis + `normLang` son live; luego 1-2 ciclos + `/verify`. **🔴 ROTAR credenciales** (PAT Airtable, service_role Supabase, Anthropic, Supadata — vencido). **Equipo:** sembrar 3-5 referentes TikTok, aprobar Referentes propuestos. **Commit:** los 12 archivos quedan staged sin commitear (Mani decide cuándo).

**2026-07-14 (cierre 37) — Métricas dejada lista + costos en $ (Supadata/Haiku ya vivos, Apify implementado) + página "Costos" + contadores Apify por actor en los 3 workflows (Mani + Claude).** Auditoría de la parte de Métricas contra el contrato ([airtable-cockpit.md §8](../../core/contracts/airtable-cockpit.md) + [ADR-021 bis](../adr/ADR-021-medicion-desempeno-embudo.md)) y cambios por API sobre la base viva `appkdNLlN1v6XdKHn`. **Hecho:** (1) **`diagnostico` backfilleado** en las 6 filas de proyecto del ciclo 07-13 (estaban vacías; usé la regla exacta del archivado, sin IA — el archivado sólo escribe filas nuevas, nunca reescribe viejas). (2) **Costo en $ activado** (ADR-021 bis pto 2): 4 columnas-fórmula `costo_supadata`/`costo_haiku_lotes`/`costo_haiku_traducciones`/`costo_total` en Métricas (~$2.13/sem). **Tarifas confirmadas por Mani:** Supadata `$0.009`/créd, Haiku lote `$0.004`, Haiku traducc `$0.005`, Apify IG/perfiles `$0.0023`, Apify TikTok `$0.005`, lookalikes TT `$0.20`. (3) **Página "Costos"** (dashboard) creada vía MCP como **borrador sin publicar** en la interfaz "Cockpit Redes" (`pagjPe0IcSIx5TGXh`) — BigNumbers de costo + bar chart por semana, filtrada a GLOBAL. (4) **Contadores Apify por actor** (decisión de Mani: exacto, no aproximar): **Motor** cuenta `apify_ig`/`apify_tt`; **Descubrimiento** cuenta `perfiles_semilla`/`detalle_sugeridos`/`lookalikes_tt`; **Archivado** gana nodo **"Leer runs descubrimiento"** + suma Apify a GLOBAL + crea la **fila `DESCUBRIMIENTO`** (3er ámbito, ADR-021 bis pto 1). Métricas gana 9 columnas de conteo + 5 fórmulas de costo Apify; `costo_total` unificado a sumar todo por fila. **Archivos:** los 3 `workflow.json` (diff quirúrgico, validador **1194/0**), `airtable-cockpit.md`, `setup-airtable.mjs` (sin drift base-viva/`core/`). Tarifas guardadas en memoria (`pipeline-contenido-costos-servicios`). **PENDIENTE de Mani (manual):** (a) **re-importar los 3 workflows en n8n** — las columnas Apify quedan vacías (costo $0) hasta re-import + un ciclo completo (motor+descubrimiento+archivado); es el eslabón débil de siempre; (b) fijar **decimales** de las columnas de costo en la UI (la API no los setea en fórmulas); (c) **revisar/publicar** la página "Costos". **Sugerido próxima sesión:** `/verify` tras el re-import (confirmar apify_ig/tt en GLOBAL y la fila DESCUBRIMIENTO poblada), luego sumar a la página "Costos" el desglose Apify + DESCUBRIMIENTO.

**2026-07-14 (cierre 36) — Fase M2 (ADR-022) CONSTRUIDA: loop de aprendizaje de criterios — motor lee `criterios_aprendidos`, archivado destila + salud por referente (30→36 nodos) + página "A revisar" (Mani + Claude).** Retomamos el handoff sobre ADR-22. Confirmado con Mani: M1 ya corrió/verificó en vivo → desbloquea M2 (la regla de secuencia de refactor-relevancia §Fases M). Diseño ya lockeado en [ADR-022](../adr/ADR-022-loop-aprendizaje-criterios.md), no se re-litigó. **Decisiones de Mani (2, superadas por Claude):** (1) el lint de criterios manuales vive en **campo aparte `advertencia_criterios`** (el gate lee `criterios_aprendidos`, mezclar el lint ahí envenenaría el rubric); (2) **mínimo de muestra `min_muestra_referente`=10** (dev-only). **Construido (código, commit `97fa24d` — SOLO los 2 `workflow.json`):** **Motor** (33 nodos, sin nodos nuevos): `Armar plan` carga `criterios_aprendidos` en cada `projects[id]`; `Gate de relevancia` concatena manuales ⊕ voz ⊕ **aprendidos** en el rubric (`PATRONES APRENDIDOS`). **Archivado** (30→36 nodos): 2 sub-cadenas fail-soft colgadas de `Cerrar run` (`onError:continue`, mismo patrón que los barridos de higiene): (a) **`Destilar criterios`** (Haiku, 1 llamada/proyecto con ≥`min_muestra_destilar`=4 calificados; prioriza los 🔥 como positivos, fallback aprobados; la MISMA llamada hace el lint de forma) → **`PATCH Proyectos criterios`** (escribe `criterios_aprendidos`+`advertencia_criterios`, nunca pisa el manual); (b) **`Leer señal selección`** (Supabase `v_senal_seleccion`) → **`Leer Referentes`** (Airtable, id↔handle) → **`Computar salud referentes`** (`tasa_gate`=gate_pass/evaluados del `por_referente` de `runs.metricas` semanal; `tasa_aprobacion`=sel/calif acumulado; `videos_evaluados`; matchea handle normalizado sin @) → **`PATCH Referentes salud`**. 2 knobs dev-only en Config del archivado. **Verificación:** validador **1194/0**, 25/25 jsCode compilan (wrap async), grafo del archivado **0 refs rotas / 0 huérfanos** (36 nodos), diff del motor localizado a 2 jsCode. **`core/` (autorizado por ADR-022):** contrato cockpit (5 campos + política salud) + `setup-airtable.mjs`. **Base viva (MCP, `appkdNLlN1v6XdKHn`):** creados **Proyectos.`criterios_aprendidos`** (`fldx3E1wDN7i5AysV`)/**`advertencia_criterios`** (`fldoikxK1fa9zhiiU`); **Referentes.`tasa_gate`** (`flddNhVUd4VOTm1KR`)/**`tasa_aprobacion`** (`fldmSoJgDVGaxt1tf`)/**`videos_evaluados`** (`fldzGZoqL2NBRHAxG`). **Página "Referentes — A revisar"** creada por MCP (draft, `pag13iDkPnK8kNQmr`, interface Cockpit Redes): grid filtrado `videos_evaluados≥10 AND tasa_gate≤0.3`, sort tasa_gate asc — **PUBLICADA** en vivo (2026-07-14). Sale vacía hasta la 1ª corrida del archivado con M2. **Docs:** refactor-relevancia (Fase M2 → CÓDIGO ✅), context.md (glosario: salud por referente + advertencia), onboarding (§6 el 🔥 ahora enseña, §5.3 salud de cuentas). **🟠 CONCURRENCIA (importante): hay una sesión paralela de Mani activa (ediciones 22:00–22:06)** que dejó sin commitear **ADR-010** (enmienda: Voz = contexto, no criterio), **ADR-021** (enmienda bis: fila DESCUBRIMIENTO en Métricas + costo $ + curar páginas), **README(adr)**, y partes de **context.md/onboarding.md**. Mi M2 es **consistente** con la enmienda de ADR-010 (mi `Destilar` hace el lint de "Voz incoherente" y no crea criterios por-Voz). Por eso commiteé **solo los 2 workflow.json**; el resto del working tree (mis adiciones a `core/`+docs **+** los docs de la sesión paralela) quedó sin commitear. **⚠️ `context.md` y `onboarding.md` están MEZCLADOS** (mis líneas M2 + las de la sesión paralela): revisar antes de commitear para no pisar. La enmienda de ADR-021 planea **código nuevo en el archivado** (fila DESCUBRIMIENTO en `Computar métricas`) pero ella misma dice *"va al siguiente lote"* → cuidado al re-importar/editar el archivado desde ambas sesiones. **🔴 CREDENCIAL EXPUESTA: Mani pegó el PAT de Airtable en el chat → ROTAR** (no se usó ni se escribió en ningún archivo; el MCP ya estaba autenticado). **PENDIENTE (Mani, manual):** (1) **RE-IMPORT de motor + archivado en n8n** — mapear cred `Airtable PAT` a los 3 nodos Airtable nuevos (`PATCH Proyectos criterios`, `Leer Referentes (archivado)`, `PATCH Referentes salud`) + `Supabase Registro` a `Leer señal selección (archivado)`; llenar `<ANTHROPIC_API_KEY>` en `Destilar criterios`. Se suma a los re-imports apilados. (2) **2 ciclos reales** para verificar M2 (que `criterios_aprendidos`/`advertencia_criterios` se pueblen y que la salud por referente cargue; hecho-cuando de la fase = precisión/separación suben en las páginas de M1). (4) Coordinar el commit de los docs de la sesión paralela. (5) **Rotar credenciales** (PAT Airtable recién expuesto + arrastres). **Próximas skills:** verificar M2 tras el re-import; después **M3** (criterios asistidos, cero código) o el lote DESCUBRIMIENTO de la sesión paralela.

**2026-07-14 (cierre 35) — Audit de 4 preguntas del equipo → higiene del archivado + `diagnostico` en Métricas + onboarding/Google Doc (Mani + Claude).** Mani preguntó 4 cosas (con credenciales Airtable/Supabase para auditar en vivo): (1) qué es `estado=promovido` en Referentes propuestos y si se puede simplificar; (2) si `Métricas` crece sin cota y dónde está el histórico; (3) si la fecha del feed de Calificar es la de publicación y si videos viejos deberían caerse; (4) cómo el workflow le sugiere al equipo mejorar los criterios (mejora continua). **Respuestas (con datos de la base viva):** (1) `promovido` lo pone el workflow (saca la fila del bucket `aprobado` para no re-crear el Referente cada semana); el equipo nunca lo toca → no es carga suya, la simplificación real es **filtrar la vista a `estado=propuesto`**. (2) `Métricas` **sí crece** (append 1 fila/proyecto+GLOBAL cada domingo; 7 filas hoy, ~7/semana); es la **única tabla monótona**; el histórico canónico vive en Supabase (`runs.metricas`+`outputs`) y el Sheet, `Métricas` es proyección regenerable. (3) el campo `fecha` de Candidatos es **`createdTime`** (cuándo llegó), NO publicación — **no hay campo de fecha de publicación**; la recencia (7d) ya evita videos viejos, y el hueco real era que el archivado **solo borra decididos** → los `nuevo` sin calificar se apilaban. (4) **el loop de mejora ya está diseñado y aceptado: ADR-022/Fase M2** (destilación Haiku → `criterios_aprendidos` + lint de forma + salud por referente); NO se re-litiga. **Mani aprobó todo. Construido (archivado 24→30 nodos, `workflow.json` — NO aplica hasta re-import):** (a) **#4 Nivel 1** — `Computar métricas semana` escribe la columna nueva **`diagnostico`** en `Métricas` (🟢 sano / 🟡 mejorable / 🔴 flojo o invertido + qué hacer, **regla sin IA**, cero costo; es el semáforo de *outcome*, complementa el lint de *forma* de M2, no lo reemplaza). Campo `diagnostico` creado en la base viva (`fldKoRvuyyuTCX0UD`). (b) **#3b** — barrido de Candidatos `nuevo` > **20 días** (umbral de Mani; sin archivar, no hubo decisión). (c) **#2** — cap de `Métricas` > **12 semanas** (84d; histórico largo en Supabase). Ambos barridos cuelgan de `Cerrar run` con `onError:continue` (el run cierra pase lo que pase; reintentan el domingo siguiente), mismo patrón lote-de-10 que `Borrar Descartes del gate`. **Verificación:** validador **1194/0**, los 3 jsCode nuevos/editados compilan (wrap async), diff limpio (2 líneas fuera + 230 dentro, sin reformateo). **Docs actualizados:** contrato cockpit (campo + política de retención), README archivado (diagrama + barridos), **enmienda ADR-021 2026-07-14** (autoriza `diagnostico` + los 2 barridos), dev-doc §4 (24→30 + nodos nuevos), onboarding (§6.2 semáforo `diagnostico` + retención 12sem, §6 barrido 20d, §8.1 vista filtrada, header). **🟠 PENDIENTE MANUAL (Mani, UI interfaz Airtable — la API no edita filtros de interfaz):** (1) página *Referentes propuestos (aprobar)* → filtro `estado is propuesto`; (2) página *Feed de Calificación* → filtro `fecha within last 11 days` (opcional, el barrido 20d ya limita); (3) página *Métricas — Calidad* → agregar el campo `diagnostico` al layout. **PENDIENTE (Mani):** re-import del **archivado** en n8n (se suma al re-import en curso del cierre 34; **mapear cred `Airtable PAT` a los 4 nodos nuevos**) → recién ahí aplican los cambios. **DECISIÓN ABIERTA:** construir **ADR-022/M2** ahora (destilación Haiku, toca también el motor → otro re-import) vs aterrizar este lote primero — Claude recomienda aterrizar + verificar `diagnostico` en una corrida real, luego M2.

**2026-07-14 (cierre 34) — Dejar el pipeline listo para el equipo: diagnóstico de integridad + revisión Supabase/Airtable + onboarding actualizado + 2 fixes en vivo + higiene idioma (Mani + Claude).** Pedido de Mani: alistar todo para el equipo de redes tras el refactor a solo-referentes. **(1) Diagnóstico de integridad (estático) de los 3 workflows:** motor 33 / archivado 24 / descubrimiento 27 nodos; **0 refs rotas, 0 huérfanos, 27/27 jsCode compilan** (wrap async como n8n), **0 secretos** (placeholders `<<SUPABASE_URL>>`/`<<AIRTABLE_BASE_ID>>`/`<ANTHROPIC_API_KEY>` intactos), crons correctos (motor lun 8am, descubrimiento lun 9am, archivado dom 6pm), features recientes cableadas (top-K descartes, rama lookalike TT, dedup, executeOnce). Validador **1194/0**. *(Verificación estática; el runtime end-to-end solo se prueba en n8n → depende del re-import.)* **(2) Supabase alineado:** los 4 objetos que tocan los workflows (`runs`/`outputs`/`processed_items`/`v_senal_seleccion`) existen en el schema 001-006; índice único `outputs.external_id` respalda el upsert, la vista expone justo las columnas seleccionadas; `v_senal_tema` inerte por ADR-019 (a propósito). **Airtable alineado:** las 8 tablas + campos coinciden con el contrato; los 2 toggles nuevos (`Descubrir en IG/TikTok`) sembrados; filas keyword de ADR-019 y tabla `Keywords` **ya no existen** (limpieza hecha). **(3) Onboarding actualizado a 2026-07-14** ([onboarding-equipo-redes.md](../onboarding-equipo-redes.md)): header refrescado + **§1.1 "Las tres máquinas"** (mental model de los 3 crons en tabla, pedido explícito de Mani: "que entiendan los workflows") + **§4.1 "El layout que ven"** (menú del interface **Cockpit Redes** página por página con edit/solo-lectura + advertencia nombres de página ≠ tabla: *Calificar (feed)*=Candidatos, *Configuración Global*=Ajustes). Sin renumerar secciones (refs cruzadas intactas). **(4) 2 cambios en vivo en Airtable (MCP, autorizados por Mani nombrándolos):** descripción de `Resultados por cuenta de referente` (`recVIHVpoKGxK8jLF`) de "MAX: 30" → **"MAX: 50"** (estaba stale vs `cap_resultados_referente=50` del repo, cierre 32); toggle **`Buscar por referentes en TikTok`** (`reco7OyqYESfDomYT`) `0` → **`1`** — resuelve la incoherencia motor↔TikTok flageada en cierre 33 (ahora el motor SÍ rastreará referentes TT cuando se aprueben; con 0 cuentas TT hoy el eje corre vacío, no rompe). **(5) Higiene del campo `idioma` (causa raíz + síntomas):** el single-select de Candidatos había acumulado opciones basura (`ingles`,`ru`,`ko`,`ot`,`be`) que iba creando la IA cuando Supadata/Haiku devolvían códigos no-canónicos. **Fix de raíz en el motor:** normalizador `normLang` en `Armar candidato` que colapsa cualquier valor al set canónico `{es,en,pt,it,fr,otro}` (`ingles`→`en`, resto no-canónico→`otro`, vacío→vacío); 15/15 casos de prueba OK, 15/15 nodos compilan, validador 1194/0. **3 registros** (`ru`/`ot`/`be`) remapeados a `otro` vía MCP → las 5 opciones basura quedan sin uso. **🟠 PENDIENTE MANUAL (Mani, UI Airtable):** borrar esas 5 opciones muertas del dropdown `idioma` de Candidatos — el MCP `update_field` solo edita fórmulas, no choices de single-select. Ya están sin uso, borrarlas no nulea nada. **PENDIENTE CRÍTICO (Mani, EN CURSO):** **RE-IMPORT de LOS 3 workflows en n8n** — Mani lo hace ya. Arrastra TODO lo apilado + el `normLang` de este cierre (cambio de código, no aplica hasta importar) + refill de placeholders/credenciales. Otros arrastres: sembrar 3-5 referentes TikTok (bootstrap), aprobar propuestas, ROTAR credenciales (PAT Airtable + service_role Supabase + Anthropic + Supadata).

**2026-07-13 (cierre 33) — Descubrimiento gana eje TikTok (ADR-020 §8, enmienda): rama paralela lookalike, 24→27 nodos + 2 toggles team-facing (Mani + Claude).** Pedido de Mani: el descubrimiento era IG-only (el actor TikTok del motor, clockworks, no expone similares) — ¿quitar TikTok o sumarlo? Mani encontró la fuente que faltaba: **`dataovercoffee~tiktok-lookalike-search`** (semillas → cuentas parecidas con `score` de similitud + bio + métricas en 1 pasada, `exclude_usernames` para dedup en la fuente, `limit` como cap de costo). **Diseño (aprobado por Mani, no re-litigar):** rama TikTok **paralela e independiente** para no tocar la cadena IG (requisito). `Vetting IG → IF — hay semillas TT → (true) Apify Lookalikes TikTok → Vetting TikTok (Haiku) → Armar propuestas / (false) → Armar propuestas`. **Gotchas que muerden:** (a) el lookalike **mezcla todas las semillas** y NO atribuye por-semilla (a diferencia de IG `relatedProfiles`) → el proyecto de cada propuesta lo asigna su **vetting Haiku** contra los criterios de cada proyecto con semilla TT (adjunta solo donde `afin ≥ umbral`); (b) **sin captions** (el actor solo da bio + métricas) → vetting TT más conservador; si la calidad sale pobre, la mejora es una 2ª pasada `clockworks` para traer descripciones (no hecho, anotado). (c) **Dedup por (plataforma, handle)** (`conocidos_ig`/`conocidos_tt`): antes era platform-blind → un handle TT podía tapar un IG homónimo. (d) `Armar propuestas` lee **ambos** vettings por nombre (idioma del workflow) → cada fila arrastra su `plataforma` y `proyecto`; `Cerrar run` suma métricas globales IG+TT. **Toggles (decisión de Mani: simétricos IG+TT):** `descubrir_ig`/`descubrir_tt` (default 1) leídos vía `AJUSTE_MAP`; off ⇒ ese eje no genera semillas. Cap de costo `cap_lookalikes_tt` 15 queda **dev-only en Config** (patrón ADR-016, no se expone). **Bootstrap (clave):** con 0 referentes TT no hay semillas → doble gate (toggle + `IF — hay semillas TT`) corta la rama, costo 0. Construir el eje **NO elimina** la dependencia del equipo, la **reduce**: de "curar TikTok a mano para siempre" a "sembrar 3-5 cuentas TT una vez" y el buscador las multiplica. **Base viva (Reels Cockpit `appkdNLlN1v6XdKHn`, MCP):** 2 filas creadas en `Ajustes` (`tblyqriQ7LyuFoWo9`) — **Descubrir en Instagram** (`recIs5WgDmZntGGwE`) y **Descubrir en TikTok** (`recy9WJsNeyViaXLq`), ambas `valor=1`, `Mostrar al equipo ✓` (salen en Configuración Global). **Verificación:** validador **1194/0**, sintaxis de los 5 jsCode OK (vm), grafo 0 refs rotas / 0 huérfanos, cableado confirmado nodo a nodo. **NO ejecutado en n8n** (corre allá, necesita import + credenciales). **Docs actualizados (barrido completo pedido por Mani):** workflow.json (name + 3 nodos + Config), yaml (filters + stages + setup + cost), README del workflow, ADR-020 §8 (enmienda con el actor + diferencias con IG + bootstrap), contrato cockpit §5 (2 toggles + `cap_lookalikes_tt` en topes dev-only), dev-doc §1 (banner + tabla resumen 27 nodos) y §3 (nota, diagrama con rama TT, nodo-por-nodo renumerado 1→27, embudo en una frase, placeholders `<ANTHROPIC_API_KEY>` ahora **×2** vetting + `apifyApi` 3 nodos), context.md (glosario Referente propuesto), ROADMAP §5, PLAN §mapa (27 nodos), onboarding (§5.3 nota TikTok, §5.5 los 2 toggles nuevos con la diferencia "traer videos" vs "proponer cuentas", §8.1 propone IG+TikTok + bootstrap, §9). **🟠 INCOHERENCIA flageada (NO tocada, decisión de Mani):** en el **motor** el toggle `Buscar por referentes en TikTok` = **0** (nunca usaron TikTok). Si el descubrimiento propone y el equipo aprueba referentes TT, el motor **no los rastreará** hasta prender ese toggle. Los dos ejes (descubrir / buscar) son independientes por diseño. **PENDIENTE (Mani, manual):** (1) **RE-IMPORT del descubrimiento en n8n** — ahora arrastra ADR-020 §8: refill Config (nuevos knobs) + `<ANTHROPIC_API_KEY>` en **AMBOS** vetting (IG y TikTok) + cred `apifyApi` sirve para el actor dataovercoffee; el import del cierre 29 seguía pendiente, esto va junto. (2) **Sembrar 3-5 referentes TikTok** a mano (bootstrap) — depende del equipo de redes; sin eso la rama corre vacía (no rompe). (3) **Decidir la coherencia motor-TikTok** (prender `buscar_referente_tiktok` si van a usar TikTok en serio). (4) **Helper text + renombres de campos team-facing** en Airtable (Referentes/Referentes propuestos/los 2 toggles) — Mani lo hace en la UI; Claude le pasó la guía de qué es cada página/campo. (5) Arrastres del cierre 32 (re-import motor+archivado, aprobar propuestas, ROTAR credenciales).

**2026-07-13 (cierre 32) — Diagnóstico del 1er ciclo real de M1 + Fase Volumen/Utilidad + fixes de M1 (banda→top-K, Métricas inflada) (Mani + Claude).** Mani corrió motor + descubrimiento con outputs en dos carpetas (`outputs-main/`, `outputs-referentes/`). **Diagnóstico:** los 3 workflows del 07-13 cerraron `ok` (motor `2cd8694e`, descubrimiento `40cd6387`, archivado `903ad3d0`); **M1 aterrizó por 1ª vez** — `runs.metricas` completo (embudo + `por_referente` + `llamadas` + `descartes_expuestos`), archivado 73 archivados / 7 filas `Métricas`. Embudo motor: `colectados 100 → asignados 194 → pretrim 107 → heat 11 → gate 4 → outputs 4`, `sin_guion 0`. **El cuello es volumen de entrada** (solo ~11 videos recientes distintos de 5 cuentas IG, 0 TT; 4 scripts/corrida vs target ~100/sem). Transcripción 6/9 distintos (3 IG vacíos: casper×2, krosh×1). Descubrimiento: 5 propuestas (afinidad 0.6–0.7). **Hallazgos:** (a) `descartes_expuestos=0` en TODOS los runs (40 rechazos históricos, 0 en la banda `[0.35,0.6]`) — Haiku rechaza **bimodal** (<0.3), la banda vive en zona muerta; (b) fila GLOBAL de `Métricas` inflada **~7×** (colectados 6824 vs 953 real, runs_ok 56 vs 8) — reproducido contra Supabase: el filtro semanal SÍ anda (12 runs/953), el bug es que `Leer runs de la semana` dispara una vez por item de entrada y suma copias; (c) **no existía página para `Referentes propuestos`** → el descubrimiento corría en vacío (pendiente arrastrado del cierre 29); (d) 51 Candidatos `nuevo` sin calificar (backlog); (e) un lead-magnet (`nicholascrown` "Comment LETTER") pasó el gate por transcript técnico real. **Construido/decidido (validador 1185/0):** **(1)** página **"Referentes propuestos (aprobar)"** (`pageCywmenrNvSuce`) publicada en Cockpit Redes (tab *Pendientes*=`propuesto`, `estado` editable) — el equipo ya puede aprobar; la promoción corre en la próxima corrida del lunes. **(2) Volumen:** `resultados_referente` 20→**40** (live, Ajustes `recVIHVpoKGxK8jLF`); repo Config `cap_top_n` 30→**100** y `cap_resultados_referente` 30→**50** (`top_n`/`dias_recencia` ya estaban en 100/100 por el equipo; con Apify/Supadata pagos ya no hay tope de créditos). **(3) M1 Métricas fix** (archivado): `executeOnce:true` en `Leer runs de la semana` + dedupe por `id` en `Computar métricas semana`. **(4) M1 descartes → top-K** (decisión de Mani): el Gate expone los `cap_descartes` rechazos con **mayor score** (near-miss = más probables falsos negativos) en vez de la banda; knobs `banda_descarte_min/max` deprecados. **[ADR-021 enmendado](../adr/ADR-021-medicion-desempeno-embudo.md)** (§Enmienda 2026-07-13) + contrato cockpit §7/topes + dev-doc (gate/Config/banner/diagrama). **(5)** `.gitignore`: `outputs/`, `outputs-main/`, `outputs-referentes/` (dumps de MB con falso positivo `sk-` de URLs firmadas del CDN de IG). **PENDIENTE (Mani, manual):** (1) **RE-IMPORT de MOTOR y ARCHIVADO** en n8n (refill de placeholders `<<…>>` + `<ANTHROPIC_API_KEY>`×3 + `<SUPADATA_API_KEY>` + credenciales) — necesario: el gate top-K y el dedupe/executeOnce son cambios de código, no live-editables (el atajo "hand-editar 2 caps" ya no aplica porque cambió el gate). (2) **Execute motor** y verificar: colectados sube, *Descartes (auditar)* **se puebla** con top-K, y en el próximo archivado la fila GLOBAL de Métricas queda sana (~950, no 6824). (3) El equipo **aprueba propuestas** (página nueva) y **siembra referentes TikTok** (el eje TT sigue bloqueado sin handles; el descubrimiento es IG-only en v1). (4) **ROTAR credenciales** (PAT Airtable + service_role Supabase re-expuestos esta sesión; + Anthropic + Supadata de arrastre). (5) La fila GLOBAL `2026-07-13` de Métricas sigue stale (6824) hasta el próximo archivado — se auto-corrige. **M2 (ADR-022) NO se arranca hasta que el re-import + una corrida verifiquen M1 limpio** (los fixes de M1 aún no corrieron en vivo); después, `/grill-with-docs` para el diseño del loop de aprendizaje.

**2026-07-10 (cierre 31, misma sesión) — 3 mejoras de robustez/costo del motor ANTES de la corrida de prueba de M1 (Mani + Claude).** Pedido de Mani: cerrar el hallazgo del Traducir + buscar mejoras que des-riesguen la prueba. **(1) `Traducir` dedupea por video** (espejo de Transcribir): 1 llamada Haiku por `external_id` distinto, el script se reparte a todas las copias del fan-out — con proyectos gemelos se pagaba 2× cada traducción (cierra el hallazgo del cierre 30). **(2) `Transcribir` gana presupuesto de tiempo** (`presupuesto_transcribir_s` 780, Config dev-only, 0 = off): si el loop serial lo excede, corta y el resto pasa SIN transcript (fail-open → SIN GUION) — ataca directo la causa probable de los 3 runs muertos del 07-10 (~15-20 min = watchdog `N8N_RUNNERS_TASK_TIMEOUT` 900s matando el nodo entero); antes = run `fallo` sin entrega, ahora = entrega degradada. OJO: mitiga el tiempo, NO el costo Supadata (el cuello free-tier sigue, ver dev-doc §2.1). **(3) `Gate de relevancia` juzga en chunks de 25 videos/llamada**: con top_n=100 el JSON de ~100 juicios excedía `max_tokens=4000` → parse fallaba → el fail-open dejaba pasar TODO sin score (gate apagado en silencio); ahora un chunk caído solo apaga su tanda. **`Resumen del run` ajustado** a los contadores reales (traducciones = videos distintos; lotes gate = chunks). **Smoke tests 17/17** (los 12 de M1 + 5 nuevos: dedup del Traducir con copias, fail-open, presupuesto corta y con 0 se desactiva, chunking 30→2 llamadas con ≤25/chunk). Validador **1338/0**, placeholders intactos, 33 nodos. Docs: dev-doc (filas 3/21/22/23/26b + mitigación del watchdog en §2.1), contrato (topes dev-only), yaml (filters + enriquecer), README motor, memoria del proyecto. **El re-import del motor pendiente del cierre 30 ahora incluye esto** (M1 + M1b van juntos en el mismo import; los pendientes del cierre 30 siguen tal cual).

**2026-07-10 (cierre 30) — ADR-021/022 firmados + Fase M1 (medición) CONSTRUIDA: motor 30→33 nodos, archivado 18→24, cockpit 6→8 tablas + 3 páginas nuevas (Mani + Claude).** Sesión doble. **(1) Grilling** (`/grill-with-docs`, disparado por "calidad de los transcripts que llegan a cada proyecto/voz") → **[ADR-021](../adr/ADR-021-medicion-desempeno-embudo.md)** (medición: tabla `Métricas` proyectada por el archivado + descartes borderline del gate visibles + histórico enriquecido; semana = semana de calificación) y **[ADR-022](../adr/ADR-022-loop-aprendizaje-criterios.md)** (loop de aprendizaje: destilación semanal de calificados a campo `criterios_aprendidos` del Proyecto + lint en la misma llamada + el 🔥 prioriza ejemplos + campos de salud en `Referentes` con poda siempre humana — **se construye DESPUÉS de verificar M1 con un ciclo real**); glosario +5 términos (Criterios aprendidos, Descarte del gate, Precisión de entrega, Separación del gate; Calificación ajustada); plan ejecutable M1→M3 en [refactor-relevancia.md](refactor-relevancia.md) §Fases M. **(2) Build de M1 (ADR-021), código completo.** **Motor** (builder): el Gate expone la banda borderline de descartes (score 0.35–0.6, cap 10; knobs dev-only `banda_descarte_min/max`/`cap_descartes` en Config) como items `_descarte` → nodos nuevos `Preparar batch Descartes` + `POST Airtable Descartes` (continue-on-fail); `Armar candidato` los filtra; nodo nuevo **`Resumen del run`** (entre Armar candidato y Cerrar run) arma `runs.metricas` completas: embudo + `sin_guion` + `descartes_expuestos` + llamadas estimadas por servicio + **`por_referente {evaluados, gate_pass}`** en videos distintos (la mitad-motor de la higiene de ADR-022). **Archivado** (builder): `Armar filas` copia `relevancia_score/razon` a `outputs.metadata` (+ keys Sheet, pueblan si se agregan los 2 encabezados a mano); cadena de métricas que corre en AMBAS ramas del IF: `Leer runs de la semana` (PostgREST `params->>workflow=eq.motor`, timestamp URL-encodeado) → `Leer Descartes del gate` → `Computar métricas semana` (defensivo: try/catch por lectura, SIEMPRE ≥1 batch — fila por proyecto con precisión/separación + fila GLOBAL con salud) → `POST Métricas` → Cerrar run (+ rama lateral que limpia los descartes); los 2 Leer con `alwaysOutputData`+continue. **`core/` (autorizado por ADR-021):** contrato cockpit 6→8 tablas (§7 `Descartes del gate`, §8 `Métricas`) + knobs de banda en la prosa de topes + fixes de prosa stale que esperaban ADR (cron diario→semanal ×2, cap_top_n 200→100); seed `setup-airtable.mjs` con las 2 tablas + helper `fecha` + link Descartes→Proyectos. **Smoke tests 12/12** (jsCode REALES con runtime simulado, data shape run 07-09): banda/cap/fail-open del gate (Haiku caído → pasan todos, 0 descartes), filtro `_descarte` en Armar candidato, batch de descartes con link/transcript/razón, Resumen con por_referente exacto, Computar métricas con separación 0.3 y falsos_negativos, y **con CERO mocks devuelve la fila GLOBAL igual** (la cadena llega a Cerrar run). Validador **1326/0**, placeholders intactos en ambos workflows, 0 refs colgadas, pinData `{}`. **Base viva (MCP, PAT re-usado — sigue A ROTAR):** tablas **`Descartes del gate`** (`tblgxDP0J6lg3YCtW`) y **`Métricas`** (`tblbtf9BZsRSZ4cxf`) creadas con round-trip verificado (typecast, select por nombre, link real a Proyectos) y filas de prueba borradas; **3 páginas publicadas** en Cockpit Redes: *Métricas — Calidad* (`pagujTyqv23DrZSwt`, filtro duro ambito≠GLOBAL), *Métricas — Salud* (`pagquE4yuyIsRNK5j`, ambito=GLOBAL), *Descartes (auditar)* (`pagOwhzD3gSgTPFIM`) — las de métricas nacen solo-lectura y así se quedan (es la semántica de ADR-021). **Docs:** dev-doc (banner M1, diagramas de ambos workflows, filas 23/23b/23c/24/26b/27 motor y 9/10/17/17b–17f/18 archivado, mapa lee/escribe 8 tablas, `runs.metricas` y `outputs.metadata` actualizados), READMEs de motor y archivado, onboarding (§3 rutina +1 paso, §4 8 tablas en 3 grupos, **§6.1 auditar descartes** con el significado de "era bueno", **§6.2 páginas de métricas**), CLAUDE del motor (33 nodos). **Hallazgo de paso (NO tocado):** `Traducir` re-traduce cada copia del fan-out (no dedupea por `external_id` como sí hace Transcribir) — costo Haiku evitable; anotado como estimación en dev-doc 26b, candidato a fix chico. **PENDIENTE (Mani, manual):** (1) **re-import de MOTOR y ARCHIVADO en n8n** — el del motor acumula TODO lo apilado (ADR-018 + cap 100 + SIN GUION + ADR-019 + M1); el del archivado es el primero desde el cron semanal (cierre 24) + M1. Verificar en la 1ª corrida del motor: `Descartes del gate` recibe ≤10 filas borderline con razón y `runs.metricas` trae `sin_guion`/`llamadas`/`por_referente`; y el domingo: filas en `Métricas` que crucen con `runs.metricas` + descartes limpiados. (2) **Habilitar edición SOLO del campo `veredicto`** en la página *Descartes (auditar)* (nace solo-lectura; el MCP no configura permisos por página). (3) Avisar al equipo del hábito nuevo (§6.1, 2 min/semana). (4) Opcional: agregar encabezados `RELEVANCIA SCORE`/`RELEVANCIA RAZON` al Sheet Histórico. (5) Arrastres: diagnosticar los 3 runs caídos del 07-10 (~timeout 900s Transcribir), corrida paga de prueba del descubrimiento (cierre 29), checklist Airtable ADR-019 (borrar `Keywords`, `Candidatos.tema`, 2 filas Ajustes) + página para `Referentes propuestos`, **ROTAR credenciales** (PAT + service_role + Anthropic + Supadata). **M2 (ADR-022) NO se arranca hasta que M1 pase un ciclo real.**

**2026-07-10 (cierre 29) — ADR-020 ejecutado: motor de descubrimiento de referentes CONSTRUIDO (workflow nuevo, 24 nodos) + tabla `Referentes propuestos` creada en la base viva + smoke test integral en verde (Mani + Claude).** Cierra la consecuencia (−) de ADR-019 ("sin descubrimiento de cuentas nuevas") y concreta la Etapa A de la DIRECCIÓN 2026-06-23/#35 y ROADMAP §5.5. **Mecanismo ([ADR-020](../adr/ADR-020-motor-descubrimiento-referentes.md), no re-litigar):** semillas = referentes IG activos rankeados por `v_senal_seleccion` (fail-open sin señal) → Apify `apify~instagram-profile-scraper` 2 pasadas (perfiles semilla → `relatedProfiles`, los "sugeridos" del propio algoritmo de IG ~20/semilla; detalle de sugeridos → bio+followers+captions) → dedup ANTES de pagar (handle ya en `Referentes` activo-o-no O en `Referentes propuestos` cualquier estado — descartar es definitivo; privados fuera; ranking por frecuencia entre semillas, cap 20) → **vetting Haiku FAIL-CLOSED** (jurado estricto sobre bio+captions contra criterios Proyecto⊕Voz; a diferencia del motor: si Haiku falla NO se propone — acá el riesgo es inundar al equipo, no perder contenido) → propuestas con afinidad ≥0.6, cap 10/corrida, a la tabla nueva **`Referentes propuestos`** (estado propuesto/aprobado/descartado/promovido) → **rama de promoción**: al inicio de cada corrida los `aprobado` se crean como Referente (activo ✓, razón en notas) y quedan `promovido` — el equipo solo marca un select. Solo IG en v1 (clockworks no expone sugeridos; 0 refs TT). Cron lunes 9am Bogotá (1h después del motor, señal fresca del archivado del domingo); `runs` con `params.workflow='descubrimiento'` + barredor zombie propio; TODO fail-soft (Apify con `alwaysOutputData`+continue; run cierra `ok` con 0 propuestas). **Archivos:** `Workflows/workflow-descubrimiento-referentes/` (workflow.json 24 nodos por builder, yaml, README), ADR-020, contrato cockpit 5→6 tablas + 2 knobs (`Propuestas por corrida` 10, `Afinidad mínima de propuesta` 0.6, live por-corrida), seed `setup-airtable.mjs`, dev-doc §1, glosario (Referente propuesto/Semilla), ROADMAP §5.5 ✅, PLAN §1, CLAUDE/README. **Base viva (PAT/service_role re-expuestos, A ROTAR):** tabla `Referentes propuestos` creada (`tbljxOJzDs5HZnXZn`, link a Proyectos) + 2 filas Ajustes sembradas + **round-trip POST/GET/PATCH/DELETE verificado** con typecast y link real. **Smoke test integral en verde** (runtime n8n simulado, los 5 jsCode REALES en secuencia con config viva): semillas = los 5 refs activos (sin señal aún → fail-open OK), knobs de Ajustes aplican, dedup/frecuencia/privadas OK, vetting mockeado OK, umbral deja 2/3, fail-closed emite item `_vacio` y la cadena llega a Cerrar run, métricas defensivas evalúan incluso con CERO nodos ejecutados; filtro PostgREST del barredor probado en vivo. Validador **1308/0** (4 workflows). **🔴 HALLAZGO de paso (NO tocado, diagnosticar):** anoche quedaron **3 runs del motor** (03:06/03:27 UTC 07-10): 2 `fallo` barridos + 1 zombie `en_curso`, cada intento murió a ~15-20 min → huele a **timeout 900s de Transcribir con top_n≈100** (`N8N_RUNNERS_TASK_TIMEOUT`, ver memoria) o a la instancia corriendo la versión vieja sin los fixes; alguien (¿Jero?) estuvo corriendo el motor. **PENDIENTE (Mani, manual):** (1) **import del descubrimiento en n8n** (workflow NUEVO, no reemplaza nada: completar Config + `<ANTHROPIC_API_KEY>` del Vetting + credenciales Airtable PAT/Supabase Registro/apifyApi) y **única corrida paga de prueba** — verificar: run `descubrimiento` cierra `ok` con métricas `{semillas:5, sugeridos_unicos:~60-100, detalle:20, vetteados, propuestos:≤10, promovidos:0}`, filas `propuesto` en la tabla sin repetir handles conocidos, razones en español; después marcar 1 como `aprobado`, re-Execute y verificar que aparece en `Referentes` activo y la fila queda `promovido` (la 2ª corrida ya casi no paga Apify: los 20 del detalle quedaron deduplicados). (2) El **re-import del motor** sigue pendiente del cierre 28 (ADR-018+cap+SIN GUION+ADR-019) + diagnosticar los fallos de anoche. (3) Checklist Airtable UI de ADR-019 sigue: borrar tabla `Keywords`, `Candidatos.tema`, 2 filas Ajustes; + crear vista/página del cockpit para `Referentes propuestos` (grilla con estado) y habilitar edición al equipo. (4) **ROTAR credenciales** (PAT + service_role re-expuestos en esta sesión; Anthropic + Supadata siguen del cierre 28).**

**2026-07-10 (cierre 28) — ADR-019 ejecutado: remoción TOTAL del eje keyword, motor solo-referentes 36→30 nodos + secreto real scrubeado del snapshot v2 (Mani + Claude).** La dirección del cierre 23 hecha código. **Decisiones de Mani (4, en [ADR-019](../adr/ADR-019-remocion-total-eje-keyword.md), no re-litigar):** remoción total (no dormante — ADR-015→017 probó que lo dormante se re-litiga); sin keywords ni para trim (pre-trim Haiku+criterios y gate Proyecto⊕Voz ya cubren; keyword-trim sería feature nueva, YAGNI); Airtable se retira del contrato con borrado manual; solo remoción (el discovery de referentes, ROADMAP §5.5, se diseña aparte). **Cirugía del motor** (script builder, convención CLAUDE.md): salen 6 nodos — `Leer Keywords`, `Apify — TikTok` (hashtags), `Apify — IG Hashtag` (muerto desde ADR-015), `Merge IG`, `Merge TT` (cada Apify va directo a su Normalizador; el bug F1 era de 2 fuentes por input, y TikTok Perfil entraba por in1 = config colgable) y `Leer señal tema`. Salen: 3 knobs keyword de Config, `FLAG_KW`/`tt_hashtags`/`kw_to_proj` de `Armar plan`, reclamo keyword y `_tema` de `Asignar`, **pre-trim vuelve a mono-eje** (caption pobre → exento, deshace la rama por eje del cierre 27), `selTema`/bi-eje del Heat-score (señal solo `v_senal_seleccion`), campo `tema` de la salida. Chequeo ad-hoc de grafo: 30 nodos, 0 refs colgadas, 0 ocurrencias de "keyword" en el JSON, jsCode compila, placeholders intactos. **Supabase NO se toca** (DB inerte: `v_senal_tema` queda sin lectores; el archivado sigue copiando `tema:''` fail-safe — no se tocó). **`core/` (con ADR):** contrato cockpit 6→5 tablas (sale `Keywords`, `Candidatos.tema`, 2 filas de Ajustes, `cap_resultados_keyword`), seed `setup-airtable.mjs` limpio. **Docs:** dev-doc (banner ADR-019 + grafo + tabla renumerada 30 nodos), glosario (Keyword = tombstone; Referente = única fuente), README/CLAUDE/yaml del motor, PLAN §mapa, ROADMAP §5.5 (discovery ahora propone SOLO referentes), onboarding equipo (§5.3 Keywords eliminada, renumerado, "dos canales"). **🔴 HALLAZGO GRAVE de paso: el commit `14910d3` ("save v2") subió un snapshot v2 con la ANTHROPIC KEY REAL (×3) + Supadata key + base_id + supabase_url en claro.** Scrubeado a placeholders en el working tree (mismo criterio que v1, cierre 23), pero **la key vive en el historial de git → ROTAR Anthropic + Supadata YA** (se suman al pendiente de PAT Airtable + service_role). Validador **1206/0**. **PENDIENTE (Mani, manual):** (1) **re-import del motor en n8n** — arrastra TODO lo apilado: ADR-018 + cap 100 + marca SIN GUION + esta remoción; en la 1ª corrida verificar: solo 2 actores Apify, cadena de lecturas sin `Leer Keywords`, candidatos sin `tema` y 1 copia por video, y el **test de borde**: correr con `Buscar por referentes en Instagram = 0` para confirmar que `Merge scrapes` no se cuelga (antes el stub del IG Hashtag garantizaba la rama IG; ya no existe). (2) **Airtable UI** (la API no borra): ocultar/borrar página `Keywords` del Cockpit, borrar tabla `Keywords`, campo `Candidatos.tema`, filas de Ajustes "Buscar por keywords en TikTok" y "Resultados por keyword" (inofensivas mientras tanto: sin entrada en `AJUSTE_MAP` se ignoran). (3) **Rotar credenciales** (ahora incluye Anthropic + Supadata).

**2026-07-09 (cierre 27, misma sesión) — Auditoría de calidad del run 07-09 + 3 decisiones de Mani ejecutadas: pre-trim por eje, decisión #6 cerrada (marca SIN GUION), keywords OFF (Mani + Claude).** **Auditoría (videos distintos):** embudo `160 colectados → 132 asignados → 69 pretrim (−48%) → 69 heat (−0%) → 29 gate (−58%) → 18 aprobados por Jero = yield 11%`; el 58% del gasto Supadata+Haiku se pagó por videos que el gate tiró después. **La calidad la definen las fuentes, no el motor:** nicholascrown (8 aprobados) + abeteddymaruta (7) aportan el 83%; **martinelli_paul 0/20** (venta de señales "Comment CLUB" — el pre-trim lo mató BIEN, referente mal elegido); **krosh.ivan 0/20** (captions `😎 #hashtags` sin señal → el pre-trim caption-only lo aniquiló SIN juzgar contenido real — punto ciego); casper_smc 1/20 (edits musicales sin voz, transcripts vacíos/letras — gate lo tiró bien); **eje keyword TikTok 2/60 (3%)** y los skits de ishh_2_funny (caption puro hashtag) PASABAN el pre-trim y quemaban transcripción. **Gate revisado descarte por descarte: funciona** — su única "pérdida" son ~9 macro-analysis de nicholascrown que los criterios actuales no cubren (gap de criterios, no error; Jero aprueba 8/10 de lo que sí pasa de esa cuenta → nicho a discutir con el equipo). **Decisiones de Mani ejecutadas:** **(1) Pre-trim por eje** (`workflow.json`): caption pobre (<25 chars de texto real sin hashtags/urls/emojis) → eje **referente pasa sin juicio** (el gate juzga con transcript; en el run real: 32/40 de krosh y casper quedan exentos), eje **keyword descarte directo** sin pagar Haiku (ishh 10/10 muere gratis). **(2) Decisión #6 CERRADA:** sin transcript **pasa igual pero marcado** — `Armar candidato` antepone `⚠️ SIN GUION |` al `titulo` (visible en el feed del cockpit). **(3) Keywords OFF en la base viva:** `Buscar por keywords en TikTok = 0` en Ajustes vía API (live por-corrida, sin re-import; el codepath sigue por ADR-017 — la dirección post-MVP es solo-referentes + motor de descubrimiento, ROADMAP §5). Las voces como filtro de formato ("debe tener contenido hablado, no memes/edits") = lo edita el equipo en Airtable (aplica en el gate sobre transcript, ADR-010). **OJO: el equipo ya tocó Ajustes hoy** (recencia 80, candidatos 70). Validador **1188/0**, smoke tests con los datos reales del run. **PENDIENTE:** re-import del motor (acumula ADR-018 + cap 100 + pre-trim por eje + marca); reu con Jero (podar martinelli/krosh, sembrar referentes estilo abeteddymaruta/nicholascrown, ¿proyecto para el nicho macro?); rotar credenciales.

**2026-07-09 (cierre 26) — Run manual de Jero diagnosticado: duplicados = fan-out con proyectos gemelos → ADR-018 (un Candidato por video) + cap_top_n 30→100 (Mani + Claude).** Jero corrió el motor a mano (run `4af277b8`, 17:47 UTC, cerró `ok`) con la config nueva del equipo y reportó "cosas repetidas; entre eso y los descartados tengo 18 de los 50 que necesitamos". **Diagnóstico (outputs/ + Supabase + Airtable, credenciales re-expuestas, A ROTAR):** embudo `160 colectados → 264 asignados → 95 pretrim → 95 filtrados → 35 gate → 35 outputs`, cruza exacto con `runs.metricas`. **(1) Los duplicados NO son bug de dedup:** `processed_items` sano (69 nuevos registrados, 0 re-ofrecidos entre corridas). Son el **fan-out de ADR-013**: el equipo cargó *Trading Psychology* y *Trading fast tips* con **los mismos 5 referentes IG y 3 keywords** → cada video se evaluó 2× y 6 pasaron ambos gates = **6 pares duplicados (12/35 records)**; el run 07-07 dejó otros 2 pares (askvinh/jefferson). Los números de Jero cuadran exactos: aprobó 18/35 (17 videos distintos); de sus 17 descartes, 6 eran copias y 7 venían sin guion. **FIX → [ADR-018](../adr/ADR-018-un-candidato-por-video-dedup-salida.md) (enmienda ADR-013):** la evaluación sigue por (video, proyecto), pero `Armar candidato` emite **UNA copia por `external_id`** (gana mayor `relevancia_score`, empate por heat). Smoke test con el gate real del run: 35→29, 0 dups, gana siempre la mejor copia. **(2) La instancia n8n corre una versión VIEJA:** pasaron 69 videos distintos por un cap que el repo tenía en 30 desde el 07-07 → los fixes de cierre 25 **nunca se importaron**. **(3) `cap_top_n` 30→100 en Config:** 30 era cuello mortal para la necesidad real (50 aprobados/sem con gate-pass ~42% ⇒ hay que transcribir ~100+); 100 calza con `top_n`=100 de Ajustes y el target ~100/sem; ojo Transcribir serial ~5s/video ⇒ ~500s, bajo el timeout 900s. *(La mención `cap_top_n 200` en `core/contracts/airtable-cockpit.md:125` queda stale a propósito — regla core-solo-con-ADR, mismo criterio que el cron del cierre 24.)* **(4) Supadata empeoró:** 23/69 videos distintos SIN transcripción (casi todo IG; era 2/14 el 07-07) → 10/35 candidatos salieron por caption-fallback y Jero descartó 7 → **la decisión #6 (guion vacío) ya duele en serio, decidir en la reu**. **(5) Config del equipo para la reu con Jero:** proyectos gemelos = mismo nicho partido en 2 (con ADR-018 ya no duplica, pero gastan gate 2×; considerar merge o diferenciar criterios) y el volumen de entrada no da para 50/sem (5 refs IG, 0 TT, 3 kw ⇒ techo ~29 candidatos/corrida): sembrar más referentes (sobre todo TikTok) y/o subir `Resultados por cuenta` (Ajustes, cap 30). **Validador 1179/0** (se neutralizó un falso positivo `sk-` en `outputs/apify-igreels.json`: fragmento de URL firmada del CDN de IG, no un secreto). **PENDIENTE CRÍTICO: RE-IMPORT del motor en n8n antes de volver a correr** (sin eso ni ADR-018 ni el cap aplican); rotar PAT Airtable + service_role Supabase; decisión #6.

**2026-07-07 (cierre 25) — 1er run automático real (con la config del equipo de redes) revisado + 2 fixes de motor (Mani + Claude).** Primera corrida automática end-to-end con las modificaciones del equipo. **Run `2026-07-07 01:50` cerró `ok`, embudo coherente** (colectados 127 = 77 IG + 50 TT → asignados 189 → pretrim 114 → **filtrados 20** → gate 9 → outputs 9), cruza exacto con `outputs/` y `runs.metricas`. **9 candidatos finales:** 8 IG-referente / 1 TikTok-keyword (@thedarkhorsehq); todos EN traducidos a ES; **@askvinh domina 5/9**. **Gate sano** (tiró los @quinlanwalther off-topic de Storytelling, correcto). **2 candidatos con guion vacío** (Supadata devolvió vacío en 2/14 videos IG distintos; fail-open los mandó igual → uno pasó por caption). **Config del equipo (Airtable):** 5 proyectos, pero 🔴 **"Trading fast tips" sin fuentes** (0 kw/0 ref → muerto); 🟠 **voz "Rosario Gomez" (bienestar/maternidad) mal asignada a "Storytelling"**; **0 referentes TikTok** (el eje TT solo entra por keyword: 50 scrapeados → 1 sirve); keyword en blanco; handles cargados con `@@` (el código los limpia, no rompe). **Throughput:** la instancia viva tenía `cap_top_n=20` hand-edit (repo decía 200) = cuello real vs target ~100/sem; además el cap recortaba por **filas (video,proyecto), no videos distintos** (20 filas = 14 distintos → el fan-out se comía el cupo). **FIXES aplicados (validador 1179/0, REQUIEREN RE-IMPORT):** (1) `cap_top_n` 200→**30** en Config; (2) `Heat-score v1` recorta por **videos distintos** (dedup `external_id` antes del corte, respeta ADR-016). **Archivado:** el run del domingo `2026-07-05 23:00` quedó zombie `en_curso` = credencial **Google Sheets `EAUTH`** (token OAuth con vencimiento 7d, bloqueante #9). **Mani reconectó** → falta correr el archivado a mano para limpiar los 16 calificados que esperan + matar el zombie. **Mensaje al equipo enviado por WhatsApp** (referente-first + se evaluará motor de descubrimiento de referentes; sembrar TikTok; voz coherente; calificar durante la semana; proyecto sin fuentes = muerto). **PENDIENTE:** re-import del motor; correr archivado; rotar PAT Airtable + service_role Supabase; decisión **#6** (candidatos con guion vacío: no emitir vs mostrar caption como respaldo).

**2026-06-26 (cierre 24) — Entra a PRODUCCIÓN: archivado pasa a semanal + base de datos limpiada de pruebas (Mani + Claude).** Mani publica los workflows para que el equipo de redes (Majo/Jero) empiece a usarlos. Dos cambios. **(1) Cron del archivado: diario → semanal.** De `0 9 * * *` (diario 9am) a **`0 18 * * 0` (domingo 18:00)**, un día antes de la corrida del motor (lunes 8am) — le da al equipo toda la semana con los scripts antes de que el archivado los limpie de Airtable. Nodo renombrado "Cron — semanal (domingo 6pm)". Archivos: [`workflow-archivado/workflow.json`](../../Workflows/workflow-archivado/workflow.json) (expresión + id + nombre en connections), su [README](../../Workflows/workflow-archivado/README.md), [dev-doc.md](dev-doc.md) (orden de ejecución + nodo 1). Validador **1179/0**. ⚠️ **El cambio vive en el repo, NO en la instancia n8n viva** — el archivado sigue con el cron diario hasta que Mani **re-importe el `workflow.json`** o edite el Schedule Trigger a mano (`0 18 * * 0`). Sin ese paso no aplica. Las 2 menciones a "cron diario" en `core/contracts/airtable-cockpit.md` (líneas 146/158) NO se tocaron (regla: `core/` solo cambia con ADR; son prosa descriptiva y el presupuesto de API sigue holgado con semanal). **(2) Limpieza de datos de prueba (vía REST con PAT/service_role, A ROTAR).** Borrado todo lo generado por las V-runs para que el equipo arranque limpio. **Supabase:** `outputs` 30→0, `processed_items` 49→0, `runs` 10→0; intactos `clients`(1)/`workflows`(2)/`instances`(1). **Airtable:** `Candidatos` 12→0; intactos Proyectos(3)/Voces(1)/Keywords(4)/Referentes(7)/Ajustes(16). El Google Sheet "Histórico" lo limpia Mani a mano. **🔴 ROTAR PAT Airtable + service_role Supabase** (Mani confirmó que las rota). **Pendiente para que el cron semanal aplique: re-import del archivado en n8n.**

**2026-06-25 (cierre 23) — Audit final pre-producción + Ajustes verificados en vivo + dirección keyword reafirmada (Mani + Claude).** Audit completo del motor de cara a prod (sin código): validador **1179/0**, motor 36 / archivado 18 nodos, código cruzado contra ADR-017 (3 toggles + knob/cap keyword + piso por cuenta + barredor zombies, todo concuerda), 0 secretos reales en el árbol (placeholders intactos; snapshot v1 scrubeado). **Ajustes leídos de la base viva (16 filas, lo que el motor lee en `Leer Ajustes` cada corrida):** todos los pesos sanos (0.4/0.4/0.2/0.7), 3 toggles ON, recencia 7, candidatos 100, resultados ref/kw 20/10; **los 16 knobs tienen consumidor verificado, ninguno muerto**. El único fuera de default era `Bonus idioma extranjero`=1 (era de prueba) → **Mani lo bajó a 0.45 a mano**. Confirmado a Mani: los Ajustes son **live por-corrida** (editar Airtable basta, sin re-import); lo que NO es live son los caps dev-only del nodo `Config` (necesitan re-import). **Explicado el eje keyword para el equipo:** el actor TikTok busca por **hashtag** (no texto libre); la keyword se normaliza (quita `#`, minúsculas, **borra espacios** → frase se pega en un tag) y se matchea **literal** contra los hashtags del video; el idioma define el pool. **🎯 DIRECCIÓN reafirmada (Mani, next step post-prod):** el norte es que el workflow se alimente **solo de referentes** + un **motor de búsqueda/descubrimiento de referentes**, y que las **keywords desaparezcan por completo**. La reactivación keyword de ADR-017 es un **puente** hasta construir ese discovery (ROADMAP §5). Detalle ya vive en §Mejoras (DIRECCIÓN 2026-06-23 + #35); no se re-litiga, se construye post-MVP. **Pendiente sin cambios:** re-import + V-run del motor con los 3 ejes ON, sembrar referentes TikTok, rotar credenciales al cerrar prod.

**2026-06-25 (cierre 22) — ADR-017: motor listo para prod — keyword TikTok reactivado como toggle + 3 toggles de eje + knob/cap keyword + piso por cuenta + barredor de zombies del motor (Mani + Claude).** Se retoma la sesión rota (working tree sin commitear era la guía). **Decisiones (en [ADR-017](../adr/ADR-017-reactivar-keyword-tiktok-y-toggles-de-eje.md), firmado, no re-litigar):** **(1)** el eje **TikTok-keyword se reactiva** (`buscar_keyword_tiktok` 0→1) — deja de estar dormante (enmienda ADR-015); `v_senal_tema`/`Candidatos.tema` vuelven a alimentarse → ADR-012/013 dejan de estar dormantes. **(2) Tres toggles de eje** en `Ajustes` (no-code), default **todos ON**, binarios 1/0: `Buscar por referentes en Instagram`/`...en TikTok`/`Buscar por keywords en TikTok` (= referentes ambas plataformas + keyword solo TikTok; **no hay** IG-keyword, retirado de verdad). `Armar plan` los lee vía `pick` y gatea cada rama. **(3) Knob propio del eje keyword** `Resultados por keyword`=10 (más chico que referentes=20: descubrimiento ciego) + cap dev-only `cap_resultados_keyword`=20; `Apify — TikTok` usa `resultsPerPage=resultados_keyword` (el de perfiles sigue con `resultados_referente`). **(4) Piso por cuenta fuente** `piso_referente`=5 (Config dev-only) — enmienda el corte de ADR-016: `Armar candidato` hace round-robin hasta `piso` videos/cuenta antes de rellenar por heat global hasta `top_n` (mata el monopolio @bayavoce; fail-open con 0; no afloja `cap_top_n`). **(5) Barredor de zombies del motor**: `Abrir run` ahora taggea `params.workflow='motor'` + nodo nuevo `Barrer runs zombie` (entre Abrir run y Leer Proyectos, scoped `params->>workflow=eq.motor`) marca `fallo` runs de motor `en_curso` previos — espejo del archivado (cierre 19). **Motor 35→36 nodos.** **Código:** `workflow.json` (Config + Armar plan + Apify TikTok + Armar candidato + nodo barredor), `airtable-cockpit.md`, `setup-airtable.mjs` (seed 4 filas Ajustes), ADR-017 nuevo, índice ADR (012/013/015/016 re-etiquetados), dev-doc (banner ADR-017 + nodo 4b + filas). **Validador 1179/0.** **Sub-agent de revisión = GO** (verde, 0 bugs, 0 contradicciones con invariantes; `cap_top_n` sigue siendo techo duro, corte sigue contando videos distintos). **Base viva (PAT/service_role, A ROTAR):** las **4 filas de Ajustes ya están** cargadas (verificado); **zombie viejo `c0a6a3c3` limpiado a mano** (era pre-tagging, sin `params.workflow` → ningún barredor lo agarraba) → **0 runs `en_curso`**; **Keywords=4 activas** (Relationships/Communication/Leadership/Corporate), **Referentes=7 todos IG, 0 TikTok**, **Keywords y demás páginas del dashboard ya visibles** (Mani lo confirmó). **Pendiente manual (Mani/equipo):** **re-import + Execute** del motor en n8n (1ª corrida con keyword vivo desde ADR-015 — ojo a cuánta basura del descubrimiento ciego llega al equipo, es la consecuencia (−) del ADR); el **equipo de redes siembra referentes TikTok** (sin ellos `buscar_referente_tiktok` ON corre vacío) y edita keywords. **🔴 ROTAR PAT Airtable + service_role Supabase al cerrar producción.** **Próximo: re-import + V-run del motor con los 3 ejes ON** (`/diagnose` si algo falla).
...
> **Cierre 24:** base limpiada de pruebas (Supabase outputs/processed_items/runs → 0; Airtable Candidatos → 0;
> config intacta) y **cron del archivado pasado a semanal `0 18 * * 0`** (domingo 6pm, un día antes del motor)
> en el repo. **Falta re-import del archivado en n8n** para que el cron nuevo aplique en vivo.

- **D1** validar TZ `America/Bogota` → activar cron motor (`lunes 8am`) + cron archivado (`domingo 6pm`, `0 18 * * 0`).
...
> 3. **✅✅ ARCHIVADO VALIDADO PARA PRODUCCIÓN (cierre 19) — V3 cubierto.** Run final `687027e2` con 14
>    calificados reales (estados mezclados): cerró `ok`/`archivados:14`; Candidatos=0 tras borrado; `outputs`=30
>    (21 aprobado/9 descartado); Sheet solo aprobados; `v_senal_seleccion` aprendiendo; 0 zombies. Probado:
>    idempotencia, paginación, split de estados, barrido, cierre robusto, curación completa. **Pendiente menor
>    (no bloquea):** ejercer con datos el **caso lote 100% vacío / 100% descartado** (rama Sheet vacía → el Merge
>    `Reconvergir tras Sheet` debe dejar pasar el borrado) — ya seguro por construcción, falta la confirmación visual.
> 4. **🟡 #9 OAuth de Google Sheets (cierre 19) — bloqueante probablemente desarmado.** Mani reconectó la
>    credencial; la consent screen no muestra toggle Testing/Published → casi seguro la app es **Internal**
>    (Workspace) → los refresh tokens **no vencen** a los 7 días. **Falta confirmar:** (a) que sea Internal
...
>    queda como red de seguridad para fallos reales (OAuth/503), no para el caso vacío. **VERIFICADO ✅:** el
>    run final `687027e2` cerró `ok`/`archivados:14` y barrió a `ed663b0e` (→`fallo`); los 3 zombies del día
>    quedaron en `fallo`, 0 `en_curso`. *(Esto NO reemplaza un error workflow global de n8n para notificación
>    proactiva ante fallo; eso sigue siendo fe ciega — pero la integridad de `runs` ya queda sana.)*
>
...
> nuevas" de **#17**.
>
> **🔁 REAFIRMADA (Mani, 2026-06-25, cierre 23) — es el next step explícito tras producción.** El norte
> es claro: el workflow se alimenta **solo de referentes** + un **motor de búsqueda/descubrimiento de
> referentes** (Etapa A), y las **keywords desaparecen por completo** (no quedan ni como criterio: eso lo
> cubre `criterios_relevancia` / #34). La reactivación del eje keyword de **ADR-017 es un puente**
> consciente hasta construir este discovery, no un cambio de rumbo. Cuando el motor de referentes exista,
> se retira el toggle `buscar_keyword_tiktok`, se vacía el eje y se cierra ADR-017.
...
**Qué se hizo.** Mani empujó motor+archivado hacia producción y testeó el **workflow de archivado** por primera
vez end-to-end con candidatos reales.

- **Credencial Google Sheets, 2 fallos resueltos:** (1) OAuth `invalid_grant` (refresh token inválido/vencido)
  → *reconnect* de la credencial en n8n; la consent screen no muestra Testing/Published → casi seguro **Internal**
  (sin vencimiento de 7d) → #9 probablemente desarmado, falta confirmar Internal + que GCP no esté en los $300 free.
  (2) `503 UNAVAILABLE` (transitorio de Google) → reintento manual OK.
- **Hardening:** `retryOnFail`+`maxTries:3`+`waitBetweenTries:30000` en *Append al Sheet Histórico*. Commit `4963a6c`.
- **Auditoría del ciclo de vida (pregunta de Mani: ¿es escalable/sostenible? ¿a dónde van los descartados?).**
  Crucé el código del archivado contra la base viva (PAT/service_role, **A ROTAR**):
  - **A dónde va cada documento:** Candidatos (Airtable) = cola de trabajo, se vacía cada run (**0 records** ✅).
    `outputs` (Supabase) = histórico canónico, TODOS los decididos con su `estado`, append-only idempotente.
    Sheet = solo `aprobado/publicado`. `v_senal_*` = vistas derivadas. `processed_items` = ledger de dedup.
  - **Descartados:** → `outputs` (estado='descartado') → borrados de Airtable → **alimentan `v_senal_seleccion`
    como clase negativa** (bajan la tasa de su referente; no se tiran).
  - **Cross-checks exactos:** `outputs`=16 (12 aprobado+4 descartado) = `runs.metricas.archivados:16`;
    `v_senal_seleccion` calificados=16/seleccionados=12; **0 filas en `outputs` sin estado** → ADR-014 se sostiene.
  - **Veredicto:** el ciclo de vida **es sostenible** — nada se acumula sin dueño.
- **🐞 Única grieta — runs zombie.** 2 runs de archivado quedaron `en_curso` para siempre (`aca7608e`,
  `d3d8723c`): los intentos fallidos de hoy abrieron run → escribieron outputs → fallaron en el Append → nunca
  llegaron a *Cerrar run*. Quiebra la confiabilidad de `runs` + trazabilidad (los 16 outputs quedaron sellados
  con el `run_id` del run **fallido** por `ignore-duplicates`, no del exitoso `b65b04f6`).
- **Fix B5 (camino de error) en código.** Nodo nuevo **`Barrer runs zombie`** en `workflow-archivado/workflow.json`
  (PATCH Supabase entre *Abrir run* y *Leer Proyectos*): marca `fallo` los runs de archivado anteriores `en_curso`,
  scoped a `params->>workflow=eq.archivado` + `id=neq.<run actual>`, `onError continue`. Auto-sanador, idempotente,
  robusto a cualquier modo de fallo; en el cron diario un zombie vive ≤1 día.

**Gotchas / aprendizajes.**
- `runs.estado` solo admite `en_curso/ok/fallo/parcial` (check de `001_registro_inicial.sql:46`) — **NO existe
  `error`**. Marcar zombies con `error` rebota por constraint; usar `fallo`.
- Los runs del archivado se distinguen del motor por `params.workflow='archivado'` (no hay columna `tipo`/`workflow`
  en `runs`). Eso permite barrer solo zombies de archivado.
- El **clasificador del harness bloqueó el PATCH directo** de los 2 zombies a la DB de producción (mutación a base
...
**✅ Archivado VALIDADO PARA PRODUCCIÓN — run final `687027e2`.** Mani calificó los 14 candidatos del motor
(estados mezclados; aprobó solo 👍/🔥) y corrió. Cerró `ok`/`archivados:14` en 6s; `ed663b0e` → `fallo`
(barrido); 0 `en_curso`; Candidatos=0; `outputs`=30 (16 previo + 14 nuevos = 21 aprobado/9 descartado); Sheet solo
aprobados; `v_senal_seleccion` aprendiendo (jefferson 5/7, howtoconvince 4/6, bayavoce 3/4). **Probado end-to-end:
idempotencia, paginación, split de estados, barrido de zombies, cierre robusto, curación completa.**

**Qué quedó a medias / qué sigue.** Caso lote 100% vacío/descartado con el código nuevo (seguro por construcción,
falta confirmación visual — no bloquea). Sigue pendiente de §Próxima sesión: resto de V-runs del motor (V2/V4/V5/V6),
Fase 4 activación (crons + D2 manifest), **rotar credenciales**.
