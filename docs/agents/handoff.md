...
- 🟠 **Pasada única sobre `setup-airtable.mjs` + `airtable-cockpit.md`** (`core/`, autorizada por ADR-024
  ...
  (3) la racionalización de campos de B.3; (4) `Candidatos.fecha` es un paso manual y el barrido de
  `nuevo` viejos depende de él → base nueva sin `fecha` = barrido roto en silencio *(cierre 43)*.
...
|---|---|---|---|
| **A** Auditoría del pipeline vivo | mapa nodo/campo/página + reconciliar repo↔live + decisión §3 (ADR) | Dev 1 | 🔧 **A.1 ✅ · A.2 ✅ · A.4 ✅** · **A.3 ⬜ siguiente** (mapa de páginas) · A.5 ⬜ (la decisión §3) |
