...
> | **F3** | 🟠 **persiste:** los 40 reels IG son TODOS @bayavoce → 1 de 7 referentes aporta; los otros 6 devolvieron 0 reels (#24). | igreels owners={bayavoce:40} · candidatos IG 3× bayavoce | 🟠 |
...
> - **D3 Supabase `outputs` 🔴 + `processed_items` ✅.** `outputs`=39 (drafts acumulan, #20). **BUG:** el run nuevo persistió **0 outputs** — nodo 31 (`Reportar outputs`) hace `POST` **sin `on_conflict`** (dev-doc:124); 3 de 4 ext_ids ya existían (bayavoce del run 19:32) → **batch entero 409** → continue-on-fail → ni el TikTok nuevo entró (run igual `ok`). Empeora con la acumulación. **Fix = simetría con archivado (nodo 11, migración 005):** `on_conflict=external_id` + `resolution=ignore-duplicates`. `processed_items`=10 (6 IG+4 TT) = lo que salió de Heat-score (nodo 33), coherente.
> - **D4 Veredicto:** **bug de código** → F1/F4/F5 ✅ cerrados; **nuevo:** idempotencia del nodo 31 del motor. **config Airtable** → F3 (ver re-diagnóstico abajo), 0 referentes TikTok. **límite del actor** → F2 (IG-hashtag sin métricas). **producto** → decidir IG-hashtag.
>
> **🔴 Hallazgo F3 re-diagnosticado (cierre 13) — #24 era teoría vieja.** Existe `Split IG referentes` → Apify IG Reels corre **1× por referente** → `ig_results_limit` es **por-referente, NO global**. Los 40 reels son todos @bayavoce porque de los **3** referentes IG de "parejas" (@bayavoce, @jefferson_fisher, @howtoconvince) **solo bayavoce devolvió reels**; los otros 2 dieron 0. **Investigar próxima sesión:** ¿sin reels en 75d? ¿privados? ¿handle mal escrito? ¿el actor falló para ellos? (lead #1).
>
