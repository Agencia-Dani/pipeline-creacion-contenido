...
- 🟠 **Pasada única sobre `setup-airtable.mjs` + `airtable-cockpit.md`** (`core/`, autorizada por ADR-024
  + el ADR de A.5). Acumula **3** cosas — hacerlas juntas, no de a una
  ([mapa-campos.md §3](./mapa-campos.md)): (1) N como global → por proyecto *(ADR-024, desde cierre 40)*;
  (2) los 2 toggles del descubrimiento (`Descubrir en Instagram`/`en TikTok`) **faltan en `ajustesSeed`**
  — la base viva los tiene (creados a mano), pero una base nueva saldría sin ellos → muerde en F5;
  (3) la racionalización de campos de B.3.
...
|---|---|---|---|
| **A** Auditoría del pipeline vivo | mapa nodo/campo/página + reconciliar repo↔live + decisión §3 (ADR) | Dev 1 | 🔧 **A.1 ✅ · A.2 🔧** (sigue en [mapa-campos.md §4](./mapa-campos.md)) · A.3–A.5 ⬜ |
